...
var FB = {
  ...
  init: function () {
    ...
    // otherwise the browser will default to 320x200
    ...
    // so we can hide the address bar in our resize function
    FB.ua = navigator.userAgent.toLowerCase();
    FB.android = FB.ua.indexOf("android") > -1 ? true : false;
    FB.ios =
      FB.ua.indexOf("iphone") > -1 || FB.ua.indexOf("ipad") > -1 ? true : false;
    ...
    window.addEventListener(
      ...
      function (e) {
        e.preventDefault();
        // the event object has an array called touches, we just want the first touch
    ...
    window.addEventListener(
      "touchmove",
      function (e) {
        // we're not interested in this but prevent default behaviour
  ...
  resize: function () {
    ...
    // allowing us to scroll pass the address bar, and thus hide it.
    ...
    // we use a timeout here as some mobile browsers won't scroll
    // if there is not a small delay
